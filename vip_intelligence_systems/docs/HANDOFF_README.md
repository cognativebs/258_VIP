...
**Prepared:** 2026-08-15
**Plan artifact:** `plan/2026-08-15_intelligence_systems.iqvplan.json` — source of truth for status/sequencing.
**Migrations (applied copies):** `infra/db/migrations/20260815_11`–`16`. Package originals remain in `vip_intelligence_systems/migrations/`.
**Logic:** `@vip/intelligence` (Phase 1 built; Phase 2/3 schema/interfaces only).
...
Phase 1 logic + fixtures live in `@vip/intelligence`. Apply `infra/db/migrations/20260815_11`–`16` to Postgres next. Do not start Phase 2 scoring until Signals ingestion is confirmed live.

## Next step after this lands

Once Phase 1 migrations are applied and the six test fixtures above are seeded, the honest next question is whether Signals ingestion is actually confirmed live yet — that's the single gate on all of Phase 2. Don't start Phase 2 logic until that's verified, not assumed.
