      ...
      {snap ? (
        <div className="stack" style={{ marginTop: 18 }}>
          ...
          <article className="panel">
            ...
            {snap.collection.binder.livePages.slice(0, 8).map((p) => (
              <p key={`${p.binderName}-${p.pageTitle}`} className="muted" style={{ fontSize: 13 }}>
                {p.binderName} · {p.pageTitle || p.pageType} · {p.ownedSlots}/{p.totalSlots} ·{" "}
                {p.ripVsSinglesRecommendation}
              </p>
            ))}
