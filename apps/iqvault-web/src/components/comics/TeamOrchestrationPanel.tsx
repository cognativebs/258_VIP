...
import {
  ...
  sortRoleIds,
  teamSummary,
} from "@/lib/orchestr8/roles";
import { applyPreset, saveTeamSettings, type TeamSettings } from "@/lib/orchestr8/teamSettings";
...
export function TeamOrchestrationPanel({ settings, onChange, onClose, gatewayHealth }: Props) {
  const [draft, setDraft] = useState<TeamSettings>(() => ({
    ...settings,
    roles: [...settings.roles],
    modelOverrides: { ...(settings.modelOverrides || {}) },
  }));
  const [agents, setAgents] = useState<Orchestr8Agent[]>(FALLBACK_AGENTS);
  const [pipelineOrder, setPipelineOrder] = useState<string[]>([]);
  const [councils, setCouncils] = useState<Orchestr8Council[]>([]);
  ...
  const pickCouncil = (council: Orchestr8Council) => {
  ...
  const renderAgentChip = (agent: Orchestr8Agent) => {
    ...
    return (
      <div key={agent.id} className={`bb-team-role-chip bb-team-role-card ${active ? "active" : ""}`}>
        ...
        {active ? (
          <label className="bb-team-model-field">
            <span>Model</span>
            <select value={modelId} onChange={(e) => setDraft((d) => ({
              ...d,
              presetId: "custom",
              modelOverrides: { ...d.modelOverrides, [agent.id]: e.target.value },
            }))}>
              {models.map((m) => (
    ...
    <div className="bb-settings-overlay" onClick={onClose}>
      <div className="bb-settings-modal bb-team-modal bb-team-modal-wide" onClick={(e) => e.stopPropagation()}>
        ...
        {gatewayHealth && !gatewayHealth.ok ? (
          <div className="bb-team-warn">Orchestr8 offline — run <code>start_orchestr8.bat</code></div>
        ) : null}
        ...
        <div className="bb-team-toolbar">
          <label className="bb-settings-field">
            ...
            <select
              ...
              onChange={(e) => setDraft((d) => ({ ...d, presetId: "custom", mode: e.target.value }))}
            >
          ...
          <label className="bb-settings-field">
            ...
            <select value={groupBy} onChange={(e) => setGroupBy(e.target.value as "provider" | "tier")}>
            ...
            ? Object.entries(grouped).map(([provId, list]) =>
                list.length ? (
                  <div key={provId} className="bb-team-provider-block">
                    <div className="bb-team-provider-head" style={{ borderColor: PROVIDERS[provId]?.color }}>
                      <strong>{PROVIDERS[provId]?.label ?? provId}</strong>
                      <span className="bb-dim">{list.length} agents</span>
                    </div>
                    <div className="bb-team-role-grid bb-team-role-grid-cards">{list.map(renderAgentChip)}</div>
                  </div>
                ) : null,
              )
            : (
              <>
                {tiers.tier1.length > 0 ? (
                  <div className="bb-team-provider-block">
                    ...
                    <div className="bb-team-role-grid bb-team-role-grid-cards">{tiers.tier1.map(renderAgentChip)}</div>
                  </div>
                ) : null}
                {tiers.tier9.length > 0 ? (
                  <div className="bb-team-provider-block">
                    ...
                    <div className="bb-team-role-grid bb-team-role-grid-cards">{tiers.tier9.map(renderAgentChip)}</div>
                  </div>
                ) : null}
              </>
            )}
        ...
        <div className="bb-team-summary">
          ...
          <span className="bb-dim"> · {providerCount} provider{providerCount !== 1 ? "s" : ""}</span>
        </div>
        <div className="bb-settings-actions">
          <button type="button" className="bb-btn bb-btn-ghost" onClick={onClose}>Cancel</button>
          <button type="button" className="bb-btn bb-btn-analytics" onClick={save}>Save team</button>
        </div>
      </div>
    </div>
  );
