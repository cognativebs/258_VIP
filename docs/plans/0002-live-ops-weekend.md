...
| G3 | Pokémon inventory + range | `/collections/tcg` shows owned Binder pockets (not seeds). Recommendations / TCG rows can show a TCGplayer **range** when `pokemontcg` external ids exist. Binder point sum stays labeled low-conf. |
