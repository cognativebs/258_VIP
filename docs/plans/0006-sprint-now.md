...
|---|---|---|
| Comic pricing trustworthy | Partial | LIVE query is tighter; PriceCharting guide quotes exist in code. VALUE is still CLZ. Quotes are unverified, not solds. Trustworthy only after local pull + token + migrate + re-walk, then we judge the numbers. |
