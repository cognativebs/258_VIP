...
CREATE OR REPLACE FUNCTION vault_core.underwriting_protect_locked()
RETURNS trigger AS $$
BEGIN
    IF OLD.locked_at IS NOT NULL THEN
        RAISE EXCEPTION 'locked underwriting records are immutable';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_underwriting_protect_locked
    BEFORE UPDATE OR DELETE ON vault_core.acquisition_underwriting
    FOR EACH ROW
    EXECUTE FUNCTION vault_core.underwriting_protect_locked();
