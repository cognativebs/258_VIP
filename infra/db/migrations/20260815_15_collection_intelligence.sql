...
COMMENT ON TABLE vault_core.binder_page IS
    'Curation target for a binder page, NOT the physical Binder Vault layout in vault_tcg.binder_page (ADR 0007). museum_page = the 9-card defining chase set for one expansion. cultural_icons = a separate page_type, NOT a variant of museum_page, per explicit design rule.';
...
COMMENT ON TABLE vault_core.binder_slot IS
    'Target card for one curated slot, NOT an occupied pocket in vault_tcg.binder_slot (ADR 0007). tier drives the rip-vs-singles recommendation logic in the view below.';
