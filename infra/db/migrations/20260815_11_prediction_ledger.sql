...
CREATE TABLE vault_core.prediction (
    ...
    -- Set by trg_prediction_set_resolves_at, not a generated column:
    -- timestamptz + interval is only STABLE (TimeZone-dependent), so Postgres
    -- rejects it in GENERATED ALWAYS. Treated as a frozen forecast field below.
...
CREATE OR REPLACE FUNCTION vault_core.prediction_set_resolves_at()
RETURNS trigger AS $$
BEGIN
    NEW.resolves_at := NEW.predicted_at + make_interval(days => NEW.horizon_days);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_prediction_set_resolves_at
    BEFORE INSERT ON vault_core.prediction
    FOR EACH ROW
    EXECUTE FUNCTION vault_core.prediction_set_resolves_at();
