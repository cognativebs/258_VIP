...
import { WORKSPACES, countActiveFilters, isHighLiquidity, isPillarReview, isLotCandidate } from "./comicEngine.js";

function compactBook(r) {
  if (!r) return null;
  return {
    series: r.Series,
    issue: r["Issue Full"],
...
function describeFilters(filters, workspace) {
...
export function buildAnalyticsContext({
  ...
  const sorted = [...filtered].sort((a, b) => (b["Current Price"] ?? 0) - (a["Current Price"] ?? 0));
  ...
  return {
    ...
    activeFilter: {
      ...
      workspace: WORKSPACES.find((w) => w.id === workspace)?.label ?? "ALL",
    ...
    collectionPhilosophy: {
      pillars: [
        "Batman", "Absolute Universe", "Spider-Man", "X-Men", "Superman",
        "First Appearances", "Cover Art & Favorite Artists", "Sci-Fi",
        "Bronze & Silver Age Keys", "Investment Portfolio",
        "Good Girl / Risqué Covers", "Personal Favorites", "General Inventory (needs review)",
      ],
      goals: [
